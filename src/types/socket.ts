...
export interface ClientToServerEvents {
  // Room management
  create_room: (
    data: { nickname: string; descriptionTime?: number; discussionTime?: number },
    callback: (response: { success: boolean; roomCode?: string; error?: string }) => void
  ) => void
  join_room: (data: { roomCode: string; nickname: string }, callback: (response: { success: boolean; error?: string }) => void) => void
